...
class TaskService {
    ...
    async createTask(userId, title, description) {
        ...
        return await taskRepository.createTask(userId, title, description);
    }
    ...
    async updateTask(taskId, userId, title, description, status) {
        ...
        if (status && !validStatuses.includes(status)) {
            throw new Error('Estado da tarefa inválido. Use: pending, in_progress ou completed.');
        }

        const updatedTask = await taskRepository.updateTask(taskId, userId, title, description, status);

        if (!updatedTask) {
            throw new Error('Tarefa não encontrada ou não tens permissão para editá-la.');
        }
